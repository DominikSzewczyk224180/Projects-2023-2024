...
def get_num_classes(config_dict: dict) -> int:
    ...
    Args:
        config_dict (dict): Dictionary of model configuration.
...
def custom_colormap() -> LinearSegmentedColormap:
    """
    Create a custom colormap for visualizing heatmaps.

    Returns:
    LinearSegmentedColormap: Custom colormap for heatmaps.
...
def create_colored_heatmaps(data: np.ndarray, filename: str, cmap: LinearSegmentedColormap) -> None:
    """
    Create and save a heatmap with a custom colormap.

    Parameters:
    data (np.ndarray): Heatmap data.
    filename (str): Output filename.
    cmap (LinearSegmentedColormap): Custom colormap for the heatmap.
...
def predict_masks(model: Model, binary: bool, generator: Generator[Tuple[np.ndarray, str, np.ndarray], None, None], model_name: str, data_set: str, patch_size: int = 256) -> str:
    """
    ...
    generator (Generator): Generator for image patches.
...
def choose_model(model_name: str) -> Tuple[Model, int]:
    """
    ...
    Returns:
    Tuple[Model, int]: A tuple containing the loaded model and its type (binary or multiclass).
...
def main(model_name: str, path: str) -> str:
    """
    Main function to load a model and predict masks on a given dataset.
