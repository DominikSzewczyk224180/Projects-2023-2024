...
def calculate_metrics(y_true: np.ndarray, y_pred: np.ndarray, binary: bool) -> Dict[str, np.ndarray]:
    """
...
def evaluate_model(model: Model, generator: Generator[Tuple[np.ndarray, str, np.ndarray], None, None]) -> Dict[str, float]:
    """
    Evaluates a trained model using test data from a generator and prints the performance metrics.

    Args:
        model (Model): Trained model to be evaluated.
        generator (Generator): Generator that provides batches of test images and corresponding masks.

    Outputs:
        dict: Dictionary containing the average performance metrics for the test data.
...
def main(model: Model, test_path: str) -> None:
    """
    Main function to evaluate the model using the test generator.
