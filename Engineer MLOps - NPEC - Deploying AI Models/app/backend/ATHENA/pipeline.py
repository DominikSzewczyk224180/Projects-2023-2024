...
train_component = command(
    ...
    code=component_path,
    command="python models/model_training_2.py --use_uri --train_image_path ${{inputs.train_image_path}} --train_mask_path ${{inputs.train_mask_path}} --val_image_path ${{inputs.val_image_path}} --val_mask_path ${{inputs.val_mask_path}} --model_save_path ${{outputs.model}} --early_stopping ${{inputs.early_stopping}} --n_epochs ${{inputs.epochs}} --hyperparameter_tuning ${{inputs.hyperparameter_tuning}} --save_model ${{inputs.save_model}}",
    environment=env,
...
evaluate_component = command(
    ...
    code=component_path,
    command="python models/model_evaluation_temporary.py --use_uri --model_path ${{inputs.model}} --test_images_path ${{inputs.test_images_path}} --test_masks_path ${{inputs.test_masks_path}} --accuracy_path ${{outputs.accuracy_path}}",
    environment=env,
...
register_component = command(
    ...
    code=component_path,
    command="python models/model_register.py --model ${{inputs.model}} --accuracy ${{inputs.accuracy}}",
    environment=env,
    compute_target=compute_target_name,
)
